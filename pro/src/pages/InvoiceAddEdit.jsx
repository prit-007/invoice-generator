...
const InvoiceAddEdit = () => {
	...
	const loadInvoice = useCallback(async () => {
		...
		try {
			...
			if (response.customer_id) {
				try {
					const customerResponse = await customersApi.getById(
	...
	useEffect(() => {
		...
		const handleClickOutside = (event) => {
			...
			if (
				productDropdown &&
				!event.target.closest(".product-dropdown-container") &&
	...
	const handleEditItem = (item) => {
		...
		document
			.getElementById("product-selection")
	...
	const handleUpdateItem = async () => {
		...
		try {
			if (isEdit && editingItem.id) {
				...
				const updatedItem = await invoiceItemsApi.update(
					editingItem.id,
					updatedData
				);
				...
				existingItem && existingItem.id
					? formData.items.map((item) =>
							item.id === existingItem.id
								? {
										...item,
										quantity: item.quantity + parseFloat(productQuantity),
								  }
								: item
					  )
					: [
	...
	const itemsTableColumns = [
		...
		},
		{
			...
			render: (value) => (
				<span className="text-orange-600">{parseFloat(value) || 0}%</span>
			),
		},
		{
			key: "tax_rate",
			...
			render: (value) => (
				<span className="text-blue-600">{parseFloat(value) || 0}%</span>
			),
		},
		{
			key: "amount",
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			// Backend InvoiceCreateRequest expects discount_percentage/discount_amount (not 'discount')
			...
			const invoiceData = {
				...
				shipping_address:
					...
					Object.keys(formData.shipping_address).length > 0
			...
			toast.success(
				isEdit ? t("invoices.updateSuccess") : t("invoices.createSuccess")
			);
			navigate("/invoices");
		} catch (err) {
		...
		<div className="space-y-6">
					...
					<div className="flex items-center space-x-3">
						...
						{isEdit && formData.id && (
							<Button
								variant="outline"
								onClick={() =>
									window.open(`/invoices/${formData.id}/pdf`, "_blank")
								}
			...
			<form onSubmit={handleSubmit} className="space-y-6">
									...
									<Select
										...
										options={[
											...
											{ value: "paid", label: t("invoices.statusPaid") },
											{
												...
												label: t("invoices.statusPartiallyPaid"),
											},
											{ value: "overdue", label: t("invoices.statusOverdue") },
											{
												value: "cancelled",
												label: t("invoices.statusCancelled"),
											},
										]}
									/>
								</div>
				...
				<Card id="product-selection">
					<Card.Header>
						<Card.Title>
							{editingItem ? t("invoices.editItem") : t("invoices.items")}
						</Card.Title>
					</Card.Header>
					<Card.Content>
						{/* Add Item Form */}
												...
												{products.map((product) => (
													...
													>
														...
														<div className="text-xs text-gray-500">
															...
															{t("products.tax")}: {product.tax_rate}%
														</div>
														{product.description && (
															<div className="text-xs text-gray-500">
																{product.description}
							...
							<div className="flex justify-end">
								<div className="w-64 space-y-2">
									...
									<div className="flex justify-between">
										...
										<span className="font-medium">
											{formatAmount(formData.tax_amount)}
										</span>
									</div>
									<div className="flex justify-between pt-2 border-t">
										...
										<span className="font-bold text-blue-600">
											{formatAmount(formData.total_amount)}
										</span>
									</div>
								</div>
							</div>
