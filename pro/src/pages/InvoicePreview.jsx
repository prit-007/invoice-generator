...
const InvoicePreview = () => {
    ...
    const loadInvoice = async () => {
        ...
        try {
            ...
            if (invoiceData.customer_id) {
                const customerData = await customersApi.getById(invoiceData.customer_id);
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            const response = await fetch(`http://127.0.0.1:1969/invoices/${id}/pdf`, {
                method: 'GET',
                headers: {
                ...
                <Card.Content className="p-8">
                    ...
                    <div className="flex justify-between items-start mb-8">
                        ...
                        <div className="text-right">
                            ...
                            <h3 className="text-lg font-bold text-gray-900">Bill Generator</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div>
                            ...
                            <div className="text-sm text-gray-900">
                                <p className="font-semibold">{customer?.name || invoice.customer_name}</p>
                                ...
                                {customer?.email && <p className="mt-2">Email: {customer.email}</p>}
                        ...
                        {invoice.shipping_details && (
                            <div>
                                <h4 className="text-sm font-semibold text-gray-700 mb-3">SHIP TO:</h4>
                                <div className="text-sm text-gray-600">
                                    <p>{invoice.shipping_details.line1}</p>
                                    {invoice.shipping_details.line2 && <p>{invoice.shipping_details.line2}</p>}
                                    <p>{invoice.shipping_details.city}, {invoice.shipping_details.state} {invoice.shipping_details.pincode}</p>
                                        ...
                                        return (
                                            <tr key={index} className="border-b border-gray-100">
                                                <td className="py-3 px-4">
                                                    ...
                                                    {item.description && item.product_name !== item.description && (
                                                        <div className="text-sm text-gray-600">{item.description}</div>
                                                    )}
                                                </td>
                                                <td className="text-center py-3 px-4">{quantity}</td>
                                                <td className="text-right py-3 px-4">{formatAmount(unitPrice)}</td>
                                                <td className="text-right py-3 px-4">{discount}%</td>
                                                <td className="text-right py-3 px-4">{taxRate}%</td>
                                                <td className="text-right py-3 px-4 font-semibold">{formatAmount(totalWithTax)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                                ...
                                {invoice.amount_paid > 0 && (
                                    <>
                                        <div className="flex justify-between text-sm text-green-600">
                                            <span>Paid:</span>
                                            <span className="font-medium">{formatAmount(invoice.amount_paid)}</span>
                                        </div>
