...
const PaymentAddEdit = () => {
	...
	const loadCustomerInvoices = useCallback(async (customerId) => {
		...
		try {
			...
			const response = await invoicesApi.getAll();
			const allInvoices = Array.isArray(response)
				? response
	...
	const loadPayment = useCallback(async () => {
		...
		try {
			...
			// Format date string
			const paymentDate = response.date
			...
			if (response.invoice_id) {
				...
				setTimeout(() => {
					const invoice = customerInvoices.find(
						(inv) => inv.id === response.invoice_id
					);
					if (invoice) {
						setInvoiceSearch(
							`${invoice.invoice_number} - ₹${(
	...
	useEffect(() => {
		if (formData.customer_id) {
			...
			if (
				formData.invoice_id &&
				!filtered.some((invoice) => invoice.id === formData.invoice_id)
			) {
	...
	const paymentMethods = [
		...
		{ value: "debit_card", label: t("payments.debitCard") },
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div className="relative dropdown-container">
									...
									<input
										...
										placeholder={
											!formData.customer_id
												...
												: t("payments.searchInvoicesOrAdvance")
										}
									...
									<div className="absolute inset-y-0 right-0 flex items-center">
										{formData.invoice_id && (
											<button
												...
												title={t("payments.clearSelection")}
											>
												<svg
													className="w-4 h-4"
													fill="none"
								...
								<p className="mt-1 text-xs text-gray-500">
									{!formData.invoice_id
										? t("payments.leaveEmptyForAdvance")
										: t("payments.appliedToSelectedInvoice")}
								</p>
							</div>
