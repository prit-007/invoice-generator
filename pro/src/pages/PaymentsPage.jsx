...
const PaymentsPage = () => {
    ...
    const loadPayments = useCallback(async () => {
        ...
        try {
            ...
            setPayments(Array.isArray(response) ? response : response.data || response || []);
        } catch (err) {
    ...
    const filteredPayments = payments
        .filter(payment => {
            ...
            const referenceMatch = payment.reference?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
            const methodMatch = payment.method?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
                                ...
                                <tr key={payment.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-900">
                                            {payment.invoice_id ? (
                                                <a href={`/invoices/edit/${payment.invoice_id}`} className="text-blue-600 hover:underline">
                                                    {payment.invoice_id}
                                                </a>
                                            ) : (
                                                '-'
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPaymentMethodBadgeClass(payment.method)}`}>
                                            {payment.method || '-'}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                ...
                {filteredPayments.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="mt-1 text-sm text-gray-500">{t('payments.noPaymentsDesc')}</p>
                    </div>
                )}
            </div>
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between bg-white px-4 py-3 rounded-lg shadow">
                    <div className="flex items-center text-sm text-gray-500">
                        {t('common.showing')} {startIndex + 1} {t('common.to')} {Math.min(startIndex + itemsPerPage, filteredPayments.length)} {t('common.of')} {filteredPayments.length} {t('common.results')}
                    </div>
